"use client";
import { Search, ExternalLink, ArrowDownCircle, Bell, Users, Crown, Sparkles, GraduationCap, Globe } from "lucide-react";
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.getRegistrations().then((registrations) => {
        for (let registration of registrations) {
          ...
          console.log('Zombie Service Worker destroyed');
        }
      });
    }
    ...
    <nav className="flex items-center justify-between bg-transparent px-4 py-3 md:px-8 gap-3 md:gap-4 w-full">
      ...
      <div className="flex items-center gap-3 w-full max-w-[420px] flex-1 min-w-0 md:flex-none md:ml-0">
        <div className="relative w-full group">
          <Search className="absolute left-3 md:left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 transition-colors group-hover:text-gray-600"/>
          <input
            type="text"
            placeholder={t("What do you want to listen to?")}
            value={query}
            onChange={(e) => onQueryChange(e.target.value)}
            className="w-full rounded-full bg-white hover:bg-gray-50/80 py-2 md:py-2.5 pl-10 md:pl-11 pr-4 text-xs md:text-sm font-medium text-gray-900 shadow-sm outline-none transition-all duration-300 focus:ring-2 focus:ring-[#0b3d33]/20 border border-gray-200/80 hover:border-gray-300"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0 ml-auto">
        <div className="hidden lg:flex items-center gap-2 mr-2">
          ...
          <div className="flex items-center gap-1.5 rounded-full hover:bg-white border border-transparent hover:border-gray-200 transition-all px-2 py-1 shadow-sm mx-1">
            <Globe className="h-4 w-4 text-gray-500" />
            <select
              value={language}
              onChange={(e) => changeLanguage(e.target.value)}
              className="bg-transparent text-[13px] font-bold text-gray-600 outline-none cursor-pointer appearance-none"
            >
          ...
          <button className="flex items-center justify-center h-8 w-8 rounded-full hover:bg-white hover:text-gray-900 text-gray-500 transition-colors hover:shadow-sm">
        ...
        {user ? (
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <button onClick={onOpenAuth} className="rounded-full bg-[#0b3d33] px-4 md:px-6 py-2 md:py-2.5 text-xs md:text-sm font-bold text-white shadow-md transition hover:scale-105 active:scale-95 hover:bg-[#072a23]">Log in</button>
          </div>
        )}
      </div>
    </nav>
